import type { Metadata } from "next";
import { notFound } from "next/navigation";
import { JsonLd } from "@/components/seo/json-ld";
import { buildBreadcrumbJsonLd } from "@/lib/jsonld";
import { HenreiItemCard } from "@/components/henrei/henrei-item-card";
import { HenreiDisclaimer } from "@/components/henrei/henrei-disclaimer";
import { FurusatoCta } from "@/components/henrei/furusato-cta";
import {
  HENREI_PRICE_TIERS,
  getPriceTierByAmount,
} from "@/lib/henrei/constants";
import {
  getHenreiItemsByPrice,
  getHenreiLastUpdated,
} from "@/lib/henrei/store";
import { formatYen } from "@/lib/utils";

interface PageProps {
  params: { amount: string };
}

export function generateStaticParams() {
  return HENREI_PRICE_TIERS.map((tier) => ({
    amount: String(tier.amount),
  }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const amount = Number(params.amount);
  const tier = getPriceTierByAmount(amount);
  if (!tier) return { title: "寄付額帯" };

  return {
    title: `寄付額${tier.label}の返礼品`,
    description: `ふるさと納税の寄付額${tier.label}（${formatYen(amount)}）帯の返礼品一覧。還元率の高い順に表示。`,
    alternates: { canonical: `/henrei/price/${amount}` },
  };
}

export default async function PriceTierPage({ params }: PageProps) {
  const amount = Number(params.amount);
  const tier = getPriceTierByAmount(amount);
  if (!tier) notFound();

  const [items, lastUpdated] = await Promise.all([
    getHenreiItemsByPrice(amount),
    getHenreiLastUpdated(),
  ]);

  return (
    <div className="container py-10">
      <JsonLd
        data={buildBreadcrumbJsonLd([
          { name: "ホーム", path: "/" },
          { name: "返礼品ナビ", path: "/henrei" },
          { name: `寄付額${tier.label}`, path: `/henrei/price/${amount}` },
        ])}
      />

      <header className="mb-8">
        <h1 className="text-3xl font-bold text-primary">
          寄付額{tier.label}の返礼品
        </h1>
        <p className="mt-2 text-muted-foreground">
          {formatYen(amount * 0.8)}〜{formatYen(amount * 1.2)}の寄付額帯の返礼品を還元率の高い順に表示しています。
        </p>
      </header>

      {items.length === 0 ? (
        <p className="text-muted-foreground">該当する返礼品が見つかりませんでした。</p>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {items.map((item, i) => (
            <HenreiItemCard key={item.id} item={item} rank={i + 1} />
          ))}
        </div>
      )}

      <div className="mt-10">
        <FurusatoCta />
      </div>

      <HenreiDisclaimer lastUpdated={lastUpdated} />
    </div>
  );
}
